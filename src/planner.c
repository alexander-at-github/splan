...
// Just a dummy function for development purpose
void plan_dummy(struct domain *domain, struct problem *problem)
{
  struct state currState = problem->init;
  struct grounder *grounder = grounder_create(domain);
  while (!planner_goalMet(problem->goal, currState)) {
    struct action **cur_actions = grounder_getActions(grounder, currState);
    // Just choosing any action
    currState = apply_action(currState, cur_actions[0]);
  }
}

// Prcondition: The goal consists only of ground predicates.
bool
planner_goalMet(struct goal *goal, struct state *state)
{
  // TODO
  assert(false);

  for (size_t i = 0; i < goal->posLiterals; ++i) {
    // Check if literal is satisfied by state
    struct atom *literal = &goal->posLiterals[i];
    
  }
  for (size_t i = 0; i < goal->negLiterals; ++i) {
    // Check if literal is satisfied by state. It should not
  }
...
// Comment/Uncomment in Vim with the command 'gc'
